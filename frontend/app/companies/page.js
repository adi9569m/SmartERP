...
export default function Companies() {
  ...
  return <main className="min-h-screen p-6 md:p-10">
    <div className="mx-auto max-w-5xl">
      <header className="mb-8 flex items-center justify-between"><div><div className="text-sm font-bold uppercase tracking-[.2em] text-blue-600">SmartERP</div><h1 className="text-3xl font-bold">Select Company</h1></div><div className="flex gap-2"><Button onClick={()=>{setForm(empty);setOpen(true)}}>New Company</Button><Button variant="secondary" onClick={logout}>Logout</Button></div></header>
      <div className="grid gap-4 md:grid-cols-2">{items.map(c=><div className="card p-5" key={c.id}><h2 className="text-lg font-bold">{c.name}</h2><p className="mt-1 text-sm text-slate-500">{c.state} · FY {c.financial_year}</p><p className="mt-1 text-xs text-slate-400">{c.gst_number || "GST not specified"}</p><div className="mt-5 flex gap-2"><Button onClick={()=>select(c.id)}>Open</Button><Button variant="secondary" onClick={()=>{setForm(c);setOpen(true)}}>Edit</Button><Button variant="danger" onClick={()=>remove(c.id)}>Delete</Button></div></div>)}</div>
