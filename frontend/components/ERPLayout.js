...
export default function ERPLayout({children}) {
  ...
  return <div className="min-h-screen md:flex">
    <aside className={`${open?"block":"hidden"} fixed inset-y-0 z-40 w-72 overflow-y-auto border-r border-white/10 bg-[#0b1f3a] p-4 text-blue-50 md:sticky md:top-0 md:h-screen md:block`}>
      <div className="mb-6 px-2"><div className="text-lg font-extrabold tracking-[.18em] text-white">SMARTERP</div><div className="mt-1 truncate text-xs text-blue-100/50">{company?.name||"Loading companyâ€¦"}</div></div>
      {sections.map(([title,links])=><div className="mb-5" key={title}><p className="mb-1 px-2 text-[10px] font-bold uppercase tracking-[.2em] text-blue-100/40">{title}</p>{links.map(([label,href])=><Link key={href} href={href} onClick={()=>setOpen(false)} className={`mb-0.5 block rounded-xl px-3 py-2.5 text-sm transition ${path===href?"bg-white text-blue-950 shadow-sm":"text-blue-50/75 hover:bg-white/10 hover:text-white"}`}>{label}</Link>)}</div>)}
    </aside>
    <div className="min-w-0 flex-1">
      <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-slate-200/80 bg-white/90 px-4 backdrop-blur-xl md:px-8"><button className="md:hidden" onClick={()=>setOpen(!open)}>â˜°</button><input data-global-search placeholder="Search current tableâ€¦  Ctrl+K" className="field max-w-md" onChange={e=>window.dispatchEvent(new CustomEvent("erp-search",{detail:e.target.value}))}/><div className="ml-auto flex gap-2"><button className="text-sm text-slate-600" onClick={()=>router.push("/companies")}>Switch Company</button><button className="text-sm text-red-600" onClick={logout}>Logout</button></div></header>
      <main className="mx-auto min-h-[calc(100vh-7.5rem)] max-w-[1600px] p-4 md:p-8">{children}</main>
